...
print("在使用数据库储存账号密码时，请使用该值代替密码，尽管它也可以被解密还原，但至少比直接保存明文密码安全得多。")
...
# 创建新对象进行密文密码登录演示
password_length = input("请输入明文密码的长度：")
u2 = dutsso.User(username=username, encrypted_password=encrypted_password, password_length=password_length)
...
if login:
    ...
    print("密文方式登录失败，请确保获取密文密码时原密码输入正确，以及密码长度输入正确！")
